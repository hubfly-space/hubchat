...
func registerWebhookRoutes(mux *http.ServeMux, deps Deps) {
	...
	mux.HandleFunc("PATCH /v1/webhooks/{id}", requireCapability(deps, authorization.IntegrationManage, handleUpdateWebhook(deps)))
	mux.HandleFunc("DELETE /v1/webhooks/{id}", requireCapability(deps, authorization.IntegrationManage, handleDeleteWebhook(deps)))
