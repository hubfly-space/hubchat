// Package jobs owns the durable background queue.
//
// # Responsibilities
//
// Enqueue, lease with heartbeat, retry with backoff, dead-lettering, concurrency limits, and per-workspace fairness.
//
// # Boundary
//
// Backed by PostgreSQL, not an external broker (§8.6). Fairness is explicit: one workspace's export must not starve everyone else's notifications.
//
