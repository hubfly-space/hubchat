  ...
  return <Page>
    ...
    <PageBody width="narrow">
      {query.isLoading ? <p className="text-sm text-fg-muted">Loading notification preferences…</p> : query.error ? <EmptyState icon={Bell} title="Notification preferences unavailable" description={query.error instanceof ApiError ? query.error.message : "Could not load your notification preferences."} action={<Button variant="secondary" onClick={query.refetch}>Try again</Button>} /> : <>
        ...
        <Section title="Delivery notes"><Callout tone="info" icon={<Bell />}>In-app notifications are durable and appear in the dashboard bell. Email-enabled alerts are queued through the configured SMTP worker. Browser alerts appear while this dashboard is open when your browser allows them.</Callout><div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-md border border-line bg-inset px-3 py-2.5"><div><p className="text-xs font-medium text-fg">Browser permission</p><p className="mt-0.5 text-2xs text-fg-muted">{browserPermission === "granted" ? "Allowed" : browserPermission === "denied" ? "Blocked in browser settings" : browserPermission === "unsupported" ? "Not supported by this browser" : "Not requested"}</p></div>{browserPermission === "default" && <Button variant="secondary" size="sm" onClick={() => void window.Notification.requestPermission().then(setBrowserPermission)}>Allow browser alerts</Button>}</div>{Boolean(save.error) && <p role="alert" className="mt-3 text-sm text-danger">{save.error instanceof ApiError ? save.error.message : "Could not save notification preferences. Your current selections are still on this page."}</p>}{save.isSuccess && <p className="mt-3 text-sm text-success-text">Notification preferences saved.</p>}</Section>
      </>}
    </PageBody>
  </Page>;
}
