...
export default function ItemDetail() {
  ...
  const commentsQuery = useInfinite<FeedbackComment>(["feedback-comments", itemId], (cursor, signal) => api.get<Paginated<FeedbackComment>>("/feedback/items/" + encodeURIComponent(itemId ?? "") + "/comments?limit=25" + (cursor ? "&cursor=" + encodeURIComponent(cursor) : ""), { signal }), { enabled: Boolean(itemId) });
  const comments = { ...commentsQuery, data: { data: commentsQuery.items } };
  const links = useInfinite<FeedbackLink>(["feedback-links", itemId], (cursor, signal) => api.get<Paginated<FeedbackLink>>(`/feedback/items/${encodeURIComponent(itemId ?? "")}/links?limit=25${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`, { signal }), { enabled: Boolean(itemId) });
  ...
  return <Page>
    ...
    <PageBody width="full"><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="space-y-4"><Card><CardHeader title="Details" /><CardBody><dl><DetailRow label="Board"><Link className="text-accent-text hover:underline" to={`/feedback/boards/${current.board_id}`}>{boardName}</Link></DetailRow><DetailRow label="Type">{current.type.replace(/_/g, " ")}</DetailRow><DetailRow label="Visibility">{current.visibility}</DetailRow><DetailRow label="Votes">{current.vote_count}</DetailRow><DetailRow label="Subscribers">{current.subscriber_count}</DetailRow><DetailRow label="Created">{formatDate(current.created_at)}</DetailRow></dl></CardBody></Card><Card><CardHeader title="Linked support work" description="Connect the request to the conversation or ticket where it is being handled." /><CardBody>{links.isLoading ? <p className="text-sm text-fg-muted">Loading links…</p> : links.error ? <p className="text-sm text-danger">Could not load linked work.</p> : links.items.length === 0 ? <p className="text-sm text-fg-muted">Nothing linked yet.</p> : <><ul className="space-y-2">{links.items.map((link) => <li key={link.id} className="flex items-center justify-between gap-2 text-xs"><span className="truncate text-fg-secondary">{link.conversation_id ? `Conversation · ${link.conversation_id}` : `Ticket · ${link.ticket_id}`}</span><Button variant="ghost" size="xs" onClick={() => void api.delete(`/feedback/items/${current.id}/links/${link.id}`).then(() => links.refetch()).catch(() => {})}>Remove</Button></li>)}</ul>{links.hasMore && <Button className="mt-3" variant="secondary" size="xs" loading={links.isFetching} onClick={() => void links.fetchNext()}>Load more links</Button>}</>}</CardBody></Card><Card><CardHeader title="Subscribers" description="Status changes are delivered to subscribed customers through the configured email worker." /><CardBody><p className="text-sm text-fg-secondary">{current.subscriber_count === 0 ? "No customers are following this item yet." : `${current.subscriber_count} customer${current.subscriber_count === 1 ? " is" : "s are"} following status changes.`}</p></CardBody></Card></aside>
    </div><div className="mt-4"><Pagination hasPrevious={false} hasNext={commentsQuery.hasMore} onPrevious={() => undefined} onNext={() => void commentsQuery.fetchNext()} summary={commentsQuery.items.length + " comment" + (commentsQuery.items.length === 1 ? "" : "s") + " loaded"} /></div></PageBody>
...
function FeedbackLinkDialog({ item, onClose, onLinked }: { item: FeedbackItem; onClose: () => void; onLinked: () => void }) {
  ...
  const conversations = useInfinite<Conversation>(["feedback-link-conversations"], (cursor, signal) => api.get<Paginated<Conversation>>("/conversations?state=new,open,pending,waiting_for_customer,waiting_for_support&limit=50" + (cursor ? "&cursor=" + encodeURIComponent(cursor) : ""), { signal }), { enabled: kind === "conversation" });
  const tickets = useInfinite<Ticket>(["feedback-link-tickets"], (cursor, signal) => api.get<Paginated<Ticket>>("/tickets?status=new,open,pending,on_hold&limit=50" + (cursor ? "&cursor=" + encodeURIComponent(cursor) : ""), { signal }), { enabled: kind === "ticket" });
