...
export default function ArticleEditor() {
  ...
  useEffect(() => {
    ...
    if (!article) return;
    setTitle(article.title); setSlug(article.slug); setExcerpt(article.excerpt); setBody(article.body); setState(article.state); setScheduleAt(localDateTimeValue(article.scheduled_at)); setLanguage(article.language || "en"); setKnowledgeBaseID(article.knowledge_base_id); setCollectionID(article.collection_id ?? undefined);
  }, [articleQuery.data]);
  ...
  const input = (nextState: string): ArticleInput => ({ knowledge_base_id: knowledgeBaseID, collection_id: collectionID, title: title.trim(), slug: slug.trim().toLowerCase(), excerpt, body, state: nextState, language, seo: {}, scheduled_at: nextState === "scheduled" ? utcTimestamp(scheduleAt) : null });
