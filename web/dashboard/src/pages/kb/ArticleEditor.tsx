...
type ArticleInput = { knowledge_base_id: string; collection_id?: string; title: string; slug: string; excerpt: string; body: string; state: string; language: string; seo: Record<string, string> };
...
export default function ArticleEditor() {
  ...
  useEffect(() => {
    ...
    if (!article) return;
    setTitle(article.title); setSlug(article.slug); setExcerpt(article.excerpt); setBody(article.body); setState(article.state); setKnowledgeBaseID(article.knowledge_base_id); setCollectionID(article.collection_id ?? undefined);
  }, [articleQuery.data]);
  ...
  const save = useMutation<ArticleInput, Article>((input) => isNew ? api.post("/articles", input, { idempotencyKey: idempotencyKey() }) : api.patch(`/articles/${encodeURIComponent(articleId ?? "")}`, input), { onSuccess: (article) => { if (isNew) navigate(`/kb/articles/${article.id}`, { replace: true }); } });
  ...
  const input = (): ArticleInput => ({ knowledge_base_id: knowledgeBaseID, collection_id: collectionID, title: title.trim(), slug: slug.trim().toLowerCase(), excerpt, body, state, language: "en", seo: {} });
